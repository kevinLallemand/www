...
try {
    ...
    echo json_encode(["type" => "Error"], JSON_UNESCAPED_UNICODE);
    exit;
}
...
if ($data && isset($data->login) && isset($data->password)) {
    if ($data === null && json_last_error() !== JSON_ERROR_NONE) {
        ...
        echo json_encode(["type" => "Error", "message" => "Échec de la connexion à la base de données"]);
        exit;
    }
    ...
    if ($query->rowCount() > 0) {
        ...
        echo json_encode(["type" => "Success"], JSON_UNESCAPED_UNICODE); // Pour une connexion réussie
    } else {
